...
async function bootstrap() {
  ...
  const scenarios = [
    ...
    { title: 'TC04: Scen 2 - Fail L1, Pass L2', type: 'S2', flow: ['FAIL1', 'PASS2'] },
    { title: 'TC05: Scen 2 - Fail L1, Pass L2', type: 'S2', flow: ['FAIL1', 'PASS2'] },
    { title: 'TC06: Scen 3 - Fail L1, Fail L2, Pass L3', type: 'S3', flow: ['FAIL1', 'FAIL2', 'PASS3'] },
    { title: 'TC07: Scen 3 - Fail L1, Fail L2, Pass L3', type: 'S3', flow: ['FAIL1', 'FAIL2', 'PASS3'] },
    { title: 'TC08: Scen 4 - Fail L1, Fail L2, Fail L3 (Debit Note)', type: 'S4', flow: ['FAIL1', 'FAIL2', 'FAIL3'] },
    { title: 'TC09: Scen 4 - Fail L1, Fail L2, Fail L3 (Debit Note)', type: 'S4', flow: ['FAIL1', 'FAIL2', 'FAIL3'] },
    { title: 'TC10: Scen 5 - Partial Receipts then Pass L1', type: 'S5', flow: ['PARTIAL_GATE', 'FULL_GATE', 'PASS1'] },
  ];
  ...
  for (let i = 0; i < scenarios.length; i++) {
    ...
    await procurementSvc.stageAction(draft.id, { action: 'APPROVE', remarks: 'Indent verified. Go ahead.' }, userId);
    log.log('Approved S1 -> S2 (Store Check)');

    // S2: Store Check
    const draftWithItems = await prisma.procurement.findUnique({ where: { id: draft.id }, include: { items: true } });
    if (!draftWithItems) throw new Error('Cannot find draft with items');

    await procurementSvc.stageAction(draft.id, { 
      action: 'NOT_AVAILABLE', 
      remarks: 'No stock available. Procure new.',
      metadata: { itemChecks: [{ itemId: draftWithItems.items[0].id, shortQty: 100 }] } 
    }, userId);
    log.log('Store Check S2 NOT_AVAILABLE -> S3 (RFQ)');

    // S3: RFQ Float
    await procurementSvc.stageAction(draft.id, { action: 'APPROVE', remarks: 'Floated to 3 vendors.' }, userId);
    ...
    // S5: Negotiation
    await procurementSvc.stageAction(draft.id, { action: 'APPROVE', remarks: 'Prices negotiated.' }, userId);
    log.log('Negotiation S5 -> S6 (PO Creation)');

    // S6: PO Creation
    await procurementSvc.stageAction(draft.id, { action: 'SUBMIT', remarks: 'PO Drafted.' }, userId);
    log.log('PO Creation S6 -> S7 (PO Approval L1)');

    // S7: PO Approval L1
    await procurementSvc.stageAction(draft.id, { action: 'APPROVE', remarks: 'Approved L1.' }, userId);
    log.log('PO Approval L1 S7 -> S8 (PO Approval L2)');

    // S8: PO Approval L2
    await procurementSvc.stageAction(draft.id, { action: 'APPROVE', remarks: 'Approved L2.' }, userId);
    log.log('PO Approval L2 S8 -> S9 (Vendor Acceptance)');

    // S9: Vendor Acceptance
    await procurementSvc.stageAction(draft.id, { action: 'APPROVE', remarks: 'Vendor accepted PO.' }, userId);
    log.log('Vendor Acceptance S9 -> S10 (Vendor Follow-up)');

    // S10: Vendor Follow-up
    await procurementSvc.stageAction(draft.id, { action: 'SUBMIT', remarks: 'Material dispatched by vendor.' }, userId);
    log.log('Vendor Follow-up S10 -> S11 (Material Receipt)');

    // S11: Material Receipt (Gate Entry)
    const procWithItems = await prisma.procurement.findUnique({ where: { id: draft.id }, include: { items: true } });
    ...
    if (sc.type === 'S5') {
      const entry1 = await gateEntrySvc.createGateEntry({
        ...
        items: [{ procurementItemId: procWithItems.items[0].id, declaredQty: 40 }],
        invoicePhotoUrls: [{ fileName: 'invoice.pdf', fileUrl: 'https://example.com/invoice.pdf' }],
        materialPhotoUrls: [{ fileName: 'material.jpg', fileUrl: 'https://example.com/material.jpg' }]
      }, userId);
      let f1 = await prisma.gateEntry.findUnique({ where: { id: entry1.id }, include: { items: true } });
      await gateEntrySvc.submitQuantityCheck(entry1.id, {
      ...
      const entry2 = await gateEntrySvc.createGateEntry({
        ...
        items: [{ procurementItemId: procWithItems.items[0].id, declaredQty: 60 }],
        invoicePhotoUrls: [{ fileName: 'invoice.pdf', fileUrl: 'https://example.com/invoice.pdf' }],
        materialPhotoUrls: [{ fileName: 'material.jpg', fileUrl: 'https://example.com/material.jpg' }]
      }, userId);
      let f2 = await prisma.gateEntry.findUnique({ where: { id: entry2.id }, include: { items: true } });
      await gateEntrySvc.submitQuantityCheck(entry2.id, {
      ...
      const entry = await gateEntrySvc.createGateEntry({
        ...
        items: [{ procurementItemId: procWithItems.items[0].id, declaredQty: 100 }],
        invoicePhotoUrls: [{ fileName: 'invoice.pdf', fileUrl: 'https://example.com/invoice.pdf' }],
        materialPhotoUrls: [{ fileName: 'material.jpg', fileUrl: 'https://example.com/material.jpg' }]
      }, userId);
      let f = await prisma.gateEntry.findUnique({ where: { id: entry.id }, include: { items: true } });
      await gateEntrySvc.submitQuantityCheck(entry.id, {
    ...
    for (const step of sc.flow) {
      ...
      await inspectionSvc.processInspection(draft.id, level, {
        items: [
          {
            procurementItemId: procWithItems.items[0].id,
            status: status as any,
            remarks: status === 'REJECTED' ? ('Failed L' + level + ' quality criteria.') : ('Passed L' + level + ' checks.')
          }
        ]
      }, userId);
    ...
    const finalProc = await prisma.procurement.findUnique({ where: { id: draft.id } });
    log.log('✅ Finished ' + sc.title + '. Final Stage: ' + finalProc?.currentStage + ' (' + finalProc?.status + ')');
  }
