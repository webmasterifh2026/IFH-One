...
import { Eye, Edit3, Save, X, Loader2, AlertCircle } from 'lucide-react';
...
const QueueRow = memo(function QueueRow({
  ...
  const cardNo =
    item.bbuCode ||
    `${item.procurement.referenceNo}-${String((item.procurement.items ?? []).findIndex((i) => i.id === item.id) + 1).padStart(3, '0')}`;
  return (
    <tr
      style={{
        background: item._selected ? 'rgba(37,99,235,0.03)' : undefined,
      }}
    >
      <td
        style={{
          position: 'sticky',
          left: 0,
          background: item._selected ? 'rgba(239,246,255,1)' : 'var(--card)',
          zIndex: 5,
          textAlign: 'center',
          padding: '10px 8px',
      ...
      >
        <button
          onClick={() => onView(item.procurement.id)}
          style={{
            background: 'none',
            border: 'none',
            color: 'var(--primary)',
      ...
      <td>
        {item.procurement.stages?.[item.procurement.currentStage]?.startedAt
          ? new Date(
              item.procurement.stages[item.procurement.currentStage].startedAt!
            )
      ...
      <td>
        <StatusBadge
          status={
            item.procurement.stages?.[item.procurement.currentStage]
              ?.stageName || title
          }
        />
      </td>
      <td>
        {(item.procurement.stages?.[item.procurement.currentStage] as any)
          ?.assignedTo?.fullName ||
          item.procurement.assignedTo?.fullName ||
          '—'}
      </td>
      ...
      <td>
        {item.procurement.requiredDate
          ? formatDate(item.procurement.requiredDate)
          : '—'}
      </td>
...
export function StageQueueWorkspace({
  title,
  description,
  stage,
  slug,
}: StageQueueWorkspaceProps) {
  ...
  const handleSelectItem = useCallback((id: string, checked: boolean) => {
    setItems((prev) =>
      prev.map((item) =>
        item.id === id ? { ...item, _selected: checked } : item
      )
    );
  }, []);
  ...
  const handleBulkSubmit = async () => {
    if (!config) {
      alert('No configuration found for this stage.');
      return;
    }

    const requiredQtyById = new Map(
      selectedItems.map((i) => [i.id, Number(i.quantity) || 0])
    );
    ...
    for (const [itemId, update] of Object.entries(bulkUpdates)) {
      if (config.storeCheckQuantityReconciliation) {
        ...
        if (
          update.availableQty === '' ||
          update.availableQty === undefined ||
          isNaN(availableQty)
        ) {
          alert(
            'Available Stock Quantity is mandatory for all selected items.'
          );
          return;
        }
        if (availableQty < 0) {
          alert('Available Stock Quantity cannot be negative.');
          return;
        }
        if (availableQty > requiredQty) {
          alert('Available Stock Quantity cannot exceed Required Quantity.');
          return;
        }
      ...
      if (!update.action) {
        alert('Action is mandatory for all selected items.');
        return;
      }
      if (
        ['HOLD', 'REJECT', 'CLARIFICATION'].includes(update.action) &&
        !update.remarks?.trim()
      ) {
        alert(
          'Remarks are mandatory when putting on hold, rejecting, or requesting clarification.'
        );
        return;
      }
      if (config.requiresAssignedTo !== false) {
        if (!update.assignedToId) {
          alert('Responsible Person is mandatory for all selected items.');
          return;
        }
      }
      ...
      if (update.action === 'HOLD') {
        if (config.toFromOptions && !update.toFrom) {
          alert('To / From is mandatory when putting an item on hold.');
          return;
        }
        if (
          config.fields.some((f) => f.id === 'holdCancelled') &&
          !update.holdCancelled
        ) {
          alert('Hold / Cancelled is mandatory when putting an item on hold.');
          return;
        }
      }
      ...
      for (const field of config.fields) {
        ...
        if (field.required && !update[field.id]) {
          alert(`${field.label} is mandatory for all selected items.`);
          return;
        }
      }
    }
    setSubmitting(true);
    try {
      const updatesList = Object.entries(bulkUpdates).map(
        ([itemId, updateData]) => {
          const metadata: any = {};
          config.fields.forEach((f) => {
            metadata[f.id] = updateData[f.id];
          });
      ...
      const result = await performBulkMultiStageAction(updatesList, {
        notifyUsers: true,
      });
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          >
            {[
              ...
              {
                label: 'Average Delay',
                value: formatDelay(kpis.averageDelayHours),
                color: '#D97706',
              ...
              },
              {
                ...
                value: `${(kpis.rejectionRate || 0).toFixed(1)}%`,
                color: '#E11D48',
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  background: page === 1 ? 'var(--surface3)' : 'var(--card)',
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  background:
                    page === totalPages ? 'var(--surface3)' : 'var(--card)',
      ...
      {showBulkModal && (
        ...
        >
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {bulkResult
                    ? 'Update Summary'
                    : `Review and update fields for ${selectedItems.length} item(s).`}
                </p>
              </div>
                ...
                <>
                  ...
                  >
                    ...
                    {config &&
                      ...
                      !config.storeCheckQuantityReconciliation && (
                        <select
                          onChange={(e) => applyAllAction(e.target.value)}
                          style={{
                            padding: '6px',
                            borderRadius: 4,
                            border: '1px solid var(--border)',
                    ...
                    {config?.requiresAssignedTo !== false && (
                      <>
                        <select
                          onChange={(e) => applyAllAssignedTo(e.target.value)}
                          style={{
                            padding: '6px',
                            borderRadius: 4,
                            border: '1px solid var(--border)',
                        ...
                        >
                          ...
                          {config?.responsiblePersonOptions
                            ? config.responsiblePersonOptions.map((o) => (
                    ...
                    {config?.fields.map((f) => {
                      ...
                      return (
                        <input
                          key={f.id}
                          type={f.type}
                          placeholder={`Apply to all: ${f.label}`}
                          onBlur={(e) => {
                            const val = e.target.value;
                            setBulkUpdates((prev) => {
                              const n = { ...prev };
                    ...
                    <input
                      type="text"
                      placeholder="Apply to all: Remarks"
                      onBlur={(e) => {
                        applyAllRemarks(e.target.value);
                        e.target.value = '';
                      }}
                      style={{
                        padding: '6px 12px',
                                      ...
                                      <td style={{ padding: '8px 12px' }}>
                                        <input
                                          ...
                                          style={{
                                            ...
                                            borderRadius: 4,
                                            textAlign: 'right',
                                            border: `1px solid ${invalid || availableQty === null ? '#DC2626' : 'var(--border)'}`,
                                            outline: 'none',
                                          }}
                                        />
                                ...
                                <td style={{ padding: '8px 12px' }}>
                                  <select
                                    ...
                                    style={{
                                      ...
                                      padding: '6px',
                                      borderRadius: 4,
                                      border: `1px solid ${!u?.assignedToId ? '#DC2626' : 'var(--border)'}`,
                                    }}
                                  >
                                    <option value="">Select Person...</option>
                                    {config?.responsiblePersonOptions
                                      ? config.responsiblePersonOptions.map(
                                          (o) => (
                                            <option
                                              key={o.value}
                                              value={o.value}
                                  ...
                                  {config?.toFromOptions ? (
                                    <select
                                      ...
                                      style={{
                                        ...
                                        padding: '6px',
                                        borderRadius: 4,
                                        border: `1px solid ${u?.action === 'HOLD' && !u?.toFrom ? '#DC2626' : 'var(--border)'}`,
                                      }}
                                    >
                                  ...
                                  {f.type === 'select' ? (
                                    <select
                                      ...
                                      style={{
                                        ...
                                        padding: '6px',
                                        borderRadius: 4,
                                        border: `1px solid ${f.required && !u?.[f.id] ? '#DC2626' : 'var(--border)'}`,
                                      }}
                                    >
                                      <option value="">Select...</option>
                                      {f.options?.map((o) => (
                                  ...
                                  <td style={{ padding: '8px 12px' }}>
                                    <select
                                      ...
                                      style={{
                                        ...
                                        padding: '6px',
                                        borderRadius: 4,
                                        border: `1px solid ${!u?.action ? '#DC2626' : 'var(--border)'}`,
                                      }}
                                    >
            ...
            >
              {bulkResult ? (
                <button onClick={closeAndRefresh} className="ifh-btn-primary">
                  Done
                </button>
              ) : (
                <>
