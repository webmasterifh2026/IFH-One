...
function Section({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
      <h3 className="text-[13px] font-bold text-gray-900 uppercase tracking-wide border-b border-gray-100 pb-3 mb-5">
        {title}
      </h3>
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-5">
      <Section title="Indent Information">
        ...
        <Field
          label="Current Stage"
          value={p.currentStageName || `Stage ${p.currentStage}`}
        />
      ...
      <Section title="Project Information">
        ...
        <Field
          ...
          value={
            p.requiredDate
              ? new Date(p.requiredDate).toLocaleDateString('en-IN')
              : undefined
          }
        />
      ...
      <Section title="Technical Information">
        <Field
          ...
          value={p.certification || 'None specified'}
        />
        <Field
          label="Packing Specification"
          value={p.packingRequirement || 'Standard'}
        />
        <Field
          label="Painting Specification"
          value={p.paintingSpec || 'Standard'}
        />
        <Field label="Inspection Requirement" value={p.inspectionRequirement} />
      </Section>

      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <h3 className="text-[13px] font-bold text-gray-900 uppercase tracking-wide px-6 pt-6 pb-3">
          Item Details
        </h3>
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead className="bg-gray-50 border-y border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider min-w-[180px]">
                    Item Name
                  </th>
                  <th className="px-4 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider min-w-[200px]">
                    Description
                  </th>
                  <th className="px-4 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                    Category
                  ...
                  <th className="px-4 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider text-right">
                    Qty
                  </th>
                  <th className="px-4 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                    UOM
              ...
              <tbody className="divide-y divide-gray-100">
                {p.items.map((item: any) => (
                  <tr key={item.id} className="hover:bg-gray-50/60">
                    <td className="px-4 py-2.5 font-mono text-[12px] font-bold text-gray-800">
                      {item.itemCode || '—'}
                    </td>
                    <td className="px-4 py-2.5 text-[13px] font-semibold text-gray-900 whitespace-normal">
                      {item.itemName}
                    </td>
                    <td className="px-4 py-2.5 text-[12px] text-gray-500 whitespace-normal">
                      {item.description || '—'}
                    </td>
                    ...
                    <td className="px-4 py-2.5 text-[13px] font-bold text-gray-900 text-right">
                      {Number(item.quantity).toLocaleString()}
                    </td>
                    <td className="px-4 py-2.5 text-[12px] font-semibold text-gray-500 uppercase">
                      {item.unit || 'EA'}
                    </td>
                    <td className="px-4 py-2.5 text-[12px] text-gray-600">
                      {item.itemType || '—'}
