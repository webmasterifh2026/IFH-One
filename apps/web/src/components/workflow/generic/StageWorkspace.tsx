...
function pendingHours(procurement: Procurement, stageNumber: number): number {
  ...
  return Math.floor(
    (Date.now() - new Date(start).getTime()) / (1000 * 60 * 60)
  );
}
        ...
        <div className="p-8 max-w-[1400px] mx-auto">
          <div className="flex items-center gap-3 p-6 bg-red-50 border border-red-200 rounded-2xl">
            ...
            <div>
              ...
              <Link
                href={backHref}
                className="text-[13px] text-red-600 hover:underline mt-1 block"
              >
                ← Back to queue
              </Link>
            </div>
...
function IndentDetailsViewBody({
  ...
  const { fieldValues, itemFieldValues, validationResults } =
    useStageFieldValues(procurement, config);
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-gray-100 font-sans">
      ...
      <header className="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-2.5 flex items-center justify-between z-10">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest bg-gray-100 text-gray-500 rounded">
              {p.priority}
            </span>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            Validation: {validationResults.filter((r) => r.passed).length}/
      ...
      <div className="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex flex-wrap gap-x-12 gap-y-4">
          ...
          <div>
            ...
            <p className="text-[13px] font-semibold text-gray-800">
              {(p as any).projectName || p.projectId || '—'}
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-0.5">
      ...
      <div className="flex-1 overflow-auto bg-gray-50">
        ...
        {activeTab === 'Documents' && (
          <DocumentsTab attachments={p.attachments} />
        )}
        {activeTab === 'Workflow' && (
          <WorkflowTab
            history={p.history}
