 ...
 */
export function DecisionPanel({ fields, fieldValues, status }: DecisionSummaryProps) {
  ...
  return (
    <div className="flex-shrink-0 bg-white border-t border-gray-200 px-6 py-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-3">
        ...
        {nonRemarksFields.map((field) => (
          <div key={field.key}>
            ...
            <p className="text-[13px] font-semibold text-gray-800">{formatFieldValue(field, fieldValues[field.key])}</p>
          </div>
        ))}
      </div>
      ...
      {remarksField && (
        <div>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">{remarksField.label}</p>
          <p className="text-[13px] text-gray-700 max-w-3xl">{formatFieldValue(remarksField, fieldValues.remarks)}</p>
        </div>
      )}
