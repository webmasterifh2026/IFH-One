...
export function ProjectSelect({
  ...
  useEffect(() => {
    ...
    const performFetch = async () => {
      ...
      try {
        ...
        const cacheKey = (searchQuery || value || '__empty__').trim() || '__empty__';
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const merged = [...optionsRef.current, ...items.filter((i: ProjectOption) => !seen.has(i.projectId))];
      setOptions(merged);
  ...
  useEffect(() => { setActiveIndex(-1); }, [options, searchQuery]);

  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', fontFamily: 'var(--font-sans)' }}>
      ...
      <div
        ...
        style={{
          ...
          outline: 'none',
          boxShadow: isOpen ? '0 0 0 3px rgba(15,123,69,0.1)' : 'var(--shadow-xs)',
          minHeight: 42,
          transition: 'all 0.2s',
        ...
        <div style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {selectedOption ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontWeight: 600, fontFamily: 'var(--font-mono)', fontSize: 13, background: 'var(--surface2)', padding: '2px 6px', borderRadius: 4 }}>
          ...
          {selectedOption && !disabled && (
            <button
              ...
              style={{
                ...
                padding: 4, borderRadius: '50%', color: 'var(--text-muted)'
              }}
      ...
      {isOpen && !disabled && createPortal(
        ...
        >
          {/* Search Input */}
          <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface2)' }}>
            ...
            />
            {loading && <Loader2 size={16} color="var(--primary)" className="animate-spin" />}
          </div>
          ...
          >
                    ...
                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
                      <div style={{ fontSize: 14, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ 
                          ...
                          background: isActive ? 'rgba(15,123,69,0.1)' : 'var(--surface2)', 
            ...
            {options.length > 0 && total > 0 && (
              <div
                ...
                style={{ padding: '8px 12px', fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', cursor: hasMore ? 'pointer' : 'default' }}
              >
                {loadingMore
                  ? 'Loading more…'
                  : hasMore
