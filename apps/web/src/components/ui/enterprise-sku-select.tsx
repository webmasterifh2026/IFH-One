...
import { ChevronDown, Search, Loader2, Sparkles, Clock, History } from 'lucide-react';
  ...
  try {
    ...
    return (
      <>
        {parts.map((part, i) =>
          regex.test(part) ? (
            <mark key={i} style={{ background: 'rgba(15,123,69,0.15)', color: 'var(--primary)', fontWeight: 700, padding: '0 2px', borderRadius: 2 }}>
              {part}
            </mark>
          ) : (
...
export function EnterpriseSkuSelect({
   ...
   */
  const fetchSkus = useCallback(async (searchQuery: string, appendOffset = 0) => {
    ...
    if (isLoadMore) setLoadingMore(true); else { setLoading(true); setSearchError(false); }
    try {
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            if (!timeoutController.signal.aborted && !abortController.signal.aborted) {
              try {
                response = await fetch(url, {
                  signal: timeoutController.signal,
                  headers: fetchHeaders,
        ...
        if (res) {
          ...
          const seen = new Set<string>(isLoadMore ? optionsRef.current.map(o => o.itemCode) : []);
          ...
          if (res.items?.project) addGroup(res.items.project, 'Project Suggested');
          if (res.items?.recent) addGroup(res.items.recent, 'Recently Used');
          if (res.items?.frequent) addGroup(res.items.frequent, 'Frequently Used');

          // Fallback for general search results
          if (Array.isArray(res.items)) addGroup(res.items, '');
          else if (Array.isArray(res)) addGroup(res, '');

          newOptions = isLoadMore ? [...optionsRef.current, ...allOptions] : allOptions;
  ...
  const pick = useCallback((opt: SkuOption) => {
  ...
  const renderOptions = useCallback(() => {
    if (searchError) {
      return (
        <li style={{ padding: '24px 16px', textAlign: 'center', color: 'var(--text-muted)' }}>
          <div style={{ fontSize: 14, fontWeight: 500, color: '#B91C1C', marginBottom: 8 }}>
            Couldn&rsquo;t load SKU results.
          </div>
          <div style={{ fontSize: 13, marginBottom: 12 }}>The server may be temporarily busy.</div>
          <button
            type="button"
            onClick={() => fetchSkus(query)}
            style={{ padding: '6px 14px', fontSize: 12, fontWeight: 600, color: 'var(--primary)', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 6, cursor: 'pointer' }}
          >
            Retry
          </button>
        </li>
        ...
        <React.Fragment key={opt.value}>
          {showHeader && (
            <div style={{
              ...
              {group === 'Project Suggested' ? <Sparkles size={12} /> : 
               group === 'Recently Used' ? <Clock size={12} /> : 
               <History size={12} />}
              {group}
            </div>
          )}
          <li
            ...
            style={{
              padding: '8px 12px',
              cursor: 'pointer',
              background: highlightedIdx === i ? 'var(--surface2)' : 'transparent',
              display: 'flex',
    ...
    return (
      <>
        ...
        {items}
        {mode === 'search' && total > 0 && (
          <li style={{ padding: '8px 12px', fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', cursor: hasMore ? 'pointer' : 'default' }}
  ...
  }, [options, query, mode, isFuzzyFallback, highlightedIdx, pick, hasMore, total, loadingMore, loadMore, searchError, fetchSkus]);
    ...
    <div ref={containerRef} style={{ position: 'relative', width: '100%' }}>
      ...
      <div
        style={{
          ...
          borderColor: error ? '#dc2626' : open ? 'var(--primary)' : 'var(--border)',
        ...
        onClick={() => { if (!disabled && searchRef.current) searchRef.current.focus(); }}
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {loading && <Loader2 className="animate-spin" style={{ width: 14, height: 14, color: 'var(--primary)' }} />}
          {value && !disabled && (
            <button
              type="button"
              onClick={(e) => { e.stopPropagation(); clear(); if (searchRef.current) searchRef.current.focus(); }}
              style={{ background: 'none', border: 'none', fontSize: 16, color: 'var(--text-muted)', cursor: 'pointer', padding: '0 2px', lineHeight: 1 }}
              title="Clear"
            >
              &times;
            </button>
