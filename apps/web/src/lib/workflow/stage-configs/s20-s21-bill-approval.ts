...
function billApproval(level: 1 | 2, stageNumber: number, slug: string): StageConfig {
  return {
    stageNumber,
    slug,
    ...
    actions: [
      { action: 'APPROVE', label: 'Approve', intent: 'positive', minRemarksLength: 0 },
      { action: 'HOLD', label: 'Hold', intent: 'warning', minRemarksLength: 20 },
      { action: 'REJECT', label: 'Reject', intent: 'negative', minRemarksLength: 20 },
    ],

    validationRules: [
      { key: 'remarksProvided', label: 'Approval Remarks Provided', check: (ctx) => (ctx.fieldValues.remarks || '').trim().length > 0 },
    ],
...
export const billApprovalL1Config: StageConfig = billApproval(1, 20, 'bill-approval-l1');
export const billApprovalL2Config: StageConfig = billApproval(2, 21, 'bill-approval-l2');
