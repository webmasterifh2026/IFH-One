    ...
    <div className="p-8 max-w-[1400px] mx-auto space-y-6">
        ...
        <div>
          ...
          <p className="text-[15px] text-gray-500 mt-1.5 font-medium">
            A holistic view of all roles and their assigned permissions across the platform.
          </p>
        </div>

        <EnterpriseCard noPadding className="overflow-x-auto">
          {loading ? (
            <div className="p-16 text-center text-gray-400">Loading matrix...</div>
          ) : roles.length === 0 ? (
            <div className="p-16 text-center text-gray-500">No roles available.</div>
          ) : (
              ...
              <thead>
                <tr>
                  <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 sticky left-0 z-10 w-80 min-w-[320px]">
                    <span className="text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Module / Permission</span>
                  </th>
                  {roles.map(role => (
                    <th key={role.id} className="px-4 py-4 bg-gray-50/80 border-b border-gray-200 text-center min-w-[120px]">
                      <div className="flex flex-col items-center gap-1.5">
                        ...
                        <span className="text-[13px] font-semibold text-gray-900 line-clamp-1" title={role.name}>
                          {role.name}
                        </span>
                      </div>
                    </th>
                ...
                {Object.entries(permissionsByModule).map(([module, perms]) => (
                  <React.Fragment key={module}>
                    ...
                    <tr className="bg-gray-50/40">
                      <td className="px-6 py-3 font-semibold text-gray-900 text-[13px] sticky left-0 bg-gray-50/40 border-r border-gray-100" colSpan={1}>
                        {module}
                      </td>
                      {roles.map(role => (
                      ...
                      <tr key={p.id} className="hover:bg-gray-50/50 transition-colors">
                        <td className="px-6 py-2.5 sticky left-0 bg-white border-r border-gray-100">
                          <div className="flex flex-col">
                            ...
                            <span className="text-[11px] text-gray-400 font-mono mt-0.5">{p.key}</span>
                          </div>
                        </td>
                        {roles.map(role => {
                          const hasPerm = rolePerms[role.id]?.has(p.id);
                          return (
                            <td key={role.id} className="px-4 py-2.5 text-center border-l border-gray-50">
                              {hasPerm ? (
