...
import {
  Plus, Search, Filter, FileText, ChevronRight, ChevronLeft,
...
import { getProcurementStatusColor, formatDate, getStageDefinition } from '@/lib/procurement-stages';
...
export default function ProcurementPage() {
  ...
  const [selectedRecordsCache, setSelectedRecordsCache] = useState<Map<string, ProcurementListItem>>(new Map());
  ...
  const fetchData = useCallback(
    async (page = 1) => {
      setLoading(true);
      try {
        const result = await getProcurements({ page, search, status: statusFilter || undefined });
  ...
  const handleExportSelected = () => {
    ...
    const header = ['Reference No', 'Title', 'Current Stage', 'Status', 'Priority', 'Created'];
    const csvLines = [
      header.join(','),
      ...selected.map((r) =>
        [r.referenceNo, `"${r.title.replace(/"/g, '""')}"`, r.currentStage, r.status, r.priority, r.createdAt]
          .join(','),
      ),
    ];
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: 'Total Indents', value: meta.total, icon: FileText, color: 'text-[#0F7B45] bg-[#0F7B45]/8' },
          { label: 'In Progress', value: records.filter(r => r.status === 'IN_PROGRESS').length, icon: Clock, color: 'text-blue-600 bg-blue-50' },
          { label: 'Completed', value: records.filter(r => r.status === 'COMPLETED').length, icon: CheckCircle2, color: 'text-emerald-600 bg-emerald-50' },
          { label: 'On Hold', value: records.filter(r => r.status === 'ON_HOLD').length, icon: AlertCircle, color: 'text-yellow-600 bg-yellow-50' },
        ].map((stat) => (
          <EnterpriseCard key={stat.label} className="flex items-center gap-3 py-4">
            ...
            <div>
              <p className="text-[22px] font-bold text-gray-900 leading-none">{loading ? '—' : stat.value}</p>
              <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide mt-0.5">{stat.label}</p>
            </div>
          </EnterpriseCard>
        ))}
      </div>
      ...
      {canBulkUpdate && allOnPageSelected && !selectAllFiltered && meta.total > pageIds.length && (
        ...
        <table className="w-full text-left border-collapse ifh-table ifh-table">
          <thead>
            <tr>
              ...
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Reference</th>
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Title</th>
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Current Stage</th>
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Progress</th>
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Priority</th>
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Status</th>
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider">Created</th>
              <th className="px-6 py-4 bg-gray-50/80 border-b border-gray-200 text-[12px] font-semibold text-gray-500 uppercase tracking-wider text-right">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr>
                <td colSpan={canBulkUpdate ? 9 : 8} className="px-6 py-16 text-center text-[14px] text-gray-400">
              ...
              records.map((record) => {
                ...
                const progressPct = Math.round((record.currentStage / 23) * 100);
                const priorityClass = PRIORITY_COLORS[record.priority] || PRIORITY_COLORS.NORMAL;
                const isSelected = selectedIds.has(record.id);

                return (
                  <tr key={record.id} className={`group hover:bg-gray-50/50 transition-colors ${isSelected ? 'bg-[#0F7B45]/5' : ''}`}>
                    {canBulkUpdate && (
                    ...
                    <td className="px-6 py-4 max-w-[200px]">
                      <p className="text-[13px] font-semibold text-gray-900 truncate">{record.title}</p>
                      {record.requestedBy && (
                        <p className="text-[11px] text-gray-400 mt-0.5">by {record.requestedBy.fullName}</p>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <div className="flex-1 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all ${
                              ...
                              record.status === 'ON_HOLD' ? 'bg-yellow-400' :
                              'bg-blue-500'
                            }`}
                            style={{ width: `${record.status === 'COMPLETED' ? 100 : progressPct}%` }}
                          />
                        </div>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center h-6 px-2.5 rounded-lg text-[11px] font-bold uppercase tracking-wide ${priorityClass}`}>
                        {record.priority}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-[12px] text-gray-500">{formatDate(record.createdAt)}</span>
                    </td>
