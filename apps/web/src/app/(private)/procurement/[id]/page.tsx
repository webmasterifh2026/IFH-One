...
export default function ProcurementDetailPage() {
  ...
  const TABS: { id: ActiveTab; label: string; count?: number }[] = [
    ...
    { id: 'remarks', label: 'Remarks', count: procurement.remarks.length },
    {
      id: 'attachments',
      label: 'Attachments',
      count: procurement.attachments.length,
    },
    { id: 'history', label: 'Audit Trail', count: procurement.history.length },
  ];

  // Extract dynamic metadata from history
  const rfqMeta = procurement.history.find(
    (h) => h.stageNumber === 4 && h.metadata
  )?.metadata;
  const rfqNumber = rfqMeta ? JSON.parse(rfqMeta).rfqNumber : undefined;

  const poMeta = procurement.history.find(
    (h) => h.stageNumber === 6 && h.metadata
  )?.metadata;
  const poNumber = poMeta ? JSON.parse(poMeta).poNumber : undefined;

  const grnMeta = procurement.history.find(
    (h) => h.stageNumber === 11 && h.metadata
  )?.metadata;
  ...
  const ageInDays = Math.max(
    0,
    Math.floor(
      (Date.now() - new Date(procurement.createdAt).getTime()) /
        (1000 * 60 * 60 * 24)
    )
  );
  ...
  return (
    <div
      className="page-content"
      style={{ paddingBottom: 60, maxWidth: 1440, margin: '0 auto' }}
    >
      {/* ── Breadcrumb ──────────────────────────────────────────────────── */}
      <Link
        href="/procurement"
                ...
                <div className="space-y-4">
                  {procurement.remarks.length === 0 ? (
                    ...
                    procurement.remarks.map((remark) => {
                      const sdef =
                        remark.stageNumber !== null &&
                        remark.stageNumber !== undefined
          ...
          <EnterpriseCard>
            ...
            <div className="space-y-3">
              ...
              <InfoRow
                icon={User}
                label="Requested By"
                value={procurement.requestedBy.fullName}
                sub={procurement.requestedBy.employeeId}
              />
              {procurement.assignedTo && (
                <InfoRow
                  icon={User}
                  label="Assigned To"
                  value={procurement.assignedTo.fullName}
                  sub={procurement.assignedTo.employeeId}
                />
              )}
              {procurement.departmentId && (
                <InfoRow
                  icon={Building2}
                  label="Department"
                  value={procurement.departmentId}
                />
              )}
              {procurement.projectId && (
                <InfoRow
                  icon={Tag}
                  label="Project"
                  value={procurement.projectId}
                />
              )}
              <InfoRow
                icon={Calendar}
                label="Created"
                value={formatDate(procurement.createdAt)}
              />
              {procurement.completedAt && (
                <InfoRow
                  icon={Calendar}
                  label="Completed"
                  value={formatDate(procurement.completedAt)}
                />
              )}
            </div>
          </EnterpriseCard>
          ...
          <EnterpriseCard>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                {
                  label: 'Remarks',
                  value: procurement.remarks.length,
                  icon: MessageSquare,
                },
                {
                  label: 'Attachments',
                  value: procurement.attachments.length,
                  icon: Paperclip,
                },
                {
                  label: 'Events',
                  value: procurement.history.length,
                  icon: Activity,
                },
              ].map((stat) => (
                <div
