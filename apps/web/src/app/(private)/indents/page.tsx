...
export default function DraftIndentsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        'Are you sure you want to delete this draft indent? This action cannot be undone.'
      )
    )
      return;
    setDeletingId(id);
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      if (
        filterProject &&
        item.procurement.projectId !== filterProject &&
        item.procurement.projectName !== filterProject
      )
        return false;
      if (filterItemType && item.procurement.itemType !== filterItemType)
        return false;
      if (!q) return true;
      return (
  ...
  const uniqueProjects = Array.from(
    new Set(
      items
        .map((i) => i.procurement.projectName || i.procurement.projectId)
        .filter(Boolean)
    )
  );
  const uniqueItemTypes = Array.from(
    new Set(items.map((i) => i.procurement.itemType).filter(Boolean))
  );
          ...
          {loading ? (
            ...
            >
              ...
              <p style={{ fontSize: 13, marginTop: 4 }}>
                You don't have any saved drafts matching your criteria.
              </p>
              <button
                onClick={() => router.push('/indents/new')}
            ...
            <table className="ifh-table" style={{ minWidth: 3500 }}>
              ...
              <tbody>
                {filteredItems.map((item) => {
                  ...
                  const itemIdx = (item.procurement.items ?? []).findIndex(
                    (i) => i.id === item.id
                  );
                  ...
                  return (
                    <tr
                      key={`${item.procurement.id}-${item.id}`}
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <button
                            onClick={() => router.push(`/indents/${procId}`)}
                            style={{
                          ...
                          <button
                            onClick={() =>
                              router.push(`/indents/new?draftId=${procId}`)
                            }
                            style={{
                              backgroundColor: 'none',
                              border: 'none',
                          ...
                          <button
                            onClick={() => handleDuplicate(procId)}
