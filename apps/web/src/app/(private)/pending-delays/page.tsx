...
function enrichRecord(r: ReportRecord, i: number): EnrichedRecord {
  ...
  let bizOffset = 0;
  for (let s = 1; s < r.currentStage; s++)
    bizOffset += STAGE_TAT[s]?.tatHours ?? 8;
  ...
  const expectedCompletion = new Date(
    sla.deadline.getTime() + remainingStages * 2 * 86400000
  );
  const cfg = STAGE_TAT[r.currentStage];
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: active ? `0 0 0 3px ${color}22` : 'var(--shadow-xs)',
    ...
    >
      ...
      {sub && (
        <div
          style={{
            fontSize: 10,
            color: active ? 'rgba(255,255,255,0.6)' : 'var(--text-faint)',
            marginTop: 2,
          }}
        >
          {sub}
                  ...
                  (
                    ...
                    const stageName =
                      PROCUREMENT_STAGES.find(
                        (s) => s.number === r.currentStage
                      )?.name || `Stage ${r.currentStage}`;
                    return (
                      <tr
                        key={r.id}
                        style={{
                          background:
                            i % 2 === 0 ? 'var(--card)' : 'var(--surface2)',
                        }}
                      >
                        <td>
                          <a
                        ...
                        <td>
                          <span
                            style={{
                              fontWeight: 700,
                              color: sla.delayHours > 0 ? '#DC2626' : '#059669',
                            }}
                          >
                            {sla.delayHours > 0
                              ? `+${fmtBizHours(sla.delayHours)}`
                              : sla.delayHours < 0
                                ? `-${fmtBizHours(-sla.delayHours)}`
                                : 'On Time'}
                          </span>
...
export default function PendingDelaysPage() {
  ...
  const filtered = useMemo(
    () =>
      enriched.filter((e) => {
        if (filterProject && (e.record as any).projectName !== filterProject)
          return false;
        if (filterResponsible && e.responsible !== filterResponsible)
          return false;
        if (filterStage && e.record.currentStage !== Number(filterStage))
          return false;
        ...
        if (filterBucket) {
          ...
          if (filterBucket === 'at_risk' && e.sla.status !== 'at_risk')
            return false;
          if (filterBucket === 'on_time' && e.sla.status !== 'on_time')
            return false;
        }
        return true;
      }),
  ...
  const pending = filtered.filter((e) =>
    ['DRAFT', 'IN_PROGRESS'].includes(e.record.status)
  );

  // Today / this week
  const now = Date.now();
  const dayMs = 86400000;
  const delayedToday = delayed.filter(
    (e) =>
      e.sla.deadline.getTime() > now - dayMs && e.sla.deadline.getTime() <= now
  );
  const delayedWeek = delayed.filter(
    (e) =>
      e.sla.deadline.getTime() > now - 7 * dayMs &&
      e.sla.deadline.getTime() <= now
  );
  ...
  const avgBizHrs =
    filtered.length > 0
      ? filtered.reduce((s, e) => s + e.sla.bizHoursConsumed, 0) /
        filtered.length
      : 0;

  // Stage analytics
  const stageAnalytics = PROCUREMENT_STAGES.filter(
    (s) => s.number > 0 && s.number < 23
  )
    .map((s) => {
      const atStage = filtered.filter(
        (e) => e.record.currentStage === s.number
      );
      const atDelayed = atStage.filter((e) => e.sla.status === 'delayed');
      const delays = atDelayed.map((e) => e.sla.delayHours);
      const worst = atDelayed.reduce(
        (m, e) => (e.sla.delayHours > m.sla.delayHours ? e : m),
        atDelayed[0]
      );
  ...
  const byResponsible = Object.entries(
    filtered.reduce(
      (acc, e) => {
        ...
        acc[r].total++;
        if (e.sla.status === 'delayed') {
          acc[r].delayed++;
          acc[r].totalDelayH += e.sla.delayHours;
        }
        return acc;
      },
      {} as Record<
        string,
        { delayed: number; total: number; totalDelayH: number }
      >
    )
  )
    .map(([name, v]) => ({ name, ...v }))
    .sort((a, b) => b.totalDelayH - a.totalDelayH)
    .slice(0, 6);
  ...
  const uniqueProjects = [
    ...new Set(allRecords.map((r) => (r as any).projectName).filter(Boolean)),
  ];
  const allResponsibles = [
    ...new Set(Object.values(STAGE_TAT).flatMap((c) => c.responsible)),
  ].filter(Boolean);
  const hasFilter =
    filterProject ||
    filterResponsible ||
  ...
  const mgmtRows = [...filtered]
    .sort((a, b) => b.sla.delayHours - a.sla.delayHours)
    .slice(0, 20);
    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface2)' }}>
      ...
      <div
        style={{
          background: 'var(--card)',
          borderBottom: '1px solid var(--border)',
          padding: '16px max(16px, min(28px, 3vw))',
        }}
      >
        <div
          style={{
        ...
        >
          <div>
            ...
            >
              Business TAT engine — Mon–Fri + 2nd/4th/5th Sat · 09:00–18:00 ·
              9h/day
            </p>
          </div>
          <div
          ...
          >
            ...
            >
              ...
              {['DRAFT', 'IN_PROGRESS', 'ON_HOLD', 'COMPLETED', 'REJECTED'].map(
                (s) => (
      ...
      <div
        style={{
          padding: '20px max(16px, min(28px, 3vw))',
          display: 'flex',
          flexDirection: 'column',
          gap: 20,
        }}
      >
        ...
        >
          <KpiCard
            ...
            color="#DC2626"
            active={filterBucket === 'delayed'}
            onClick={() => {
              setFilterBucket('delayed');
              setDrill({ title: 'All Delayed', rows: delayed });
            }}
          />
          <KpiCard
            label="At Risk"
            value={atRisk.length}
            color="#D97706"
            active={filterBucket === 'at_risk'}
            onClick={() => {
              setFilterBucket('at_risk');
              setDrill({ title: 'At Risk', rows: atRisk });
            }}
          />
          <KpiCard
            label="On Time"
            value={onTime.length}
            color="#059669"
            active={filterBucket === 'on_time'}
            onClick={() => {
              setFilterBucket('on_time');
              setDrill({ title: 'On Time', rows: onTime });
            }}
          />
          <KpiCard
            label="Delayed Today"
            ...
            color="#EA580C"
            onClick={() =>
              setDrill({ title: 'Delayed Today', rows: delayedToday })
            }
          />
          <KpiCard
            label="Delayed This Week"
            value={delayedWeek.length}
            color="#B45309"
            onClick={() =>
              setDrill({ title: 'Delayed This Week', rows: delayedWeek })
            }
          />
          <KpiCard
                      ...
                      ({
                        ...
                        <tr
                          ...
                          onClick={() =>
                            del > 0 &&
                            setDrill({
                              title: `SLA Breached — ${stage.name}`,
                              rows: filtered.filter(
                                (e) =>
                                  e.record.currentStage === stage.number &&
                                  e.sla.status === 'delayed'
                              ),
                            })
                          }
                          onMouseEnter={(e) => {
                        ...
                        >
                          ...
                          <td>
                            <span
                              style={{
                                fontWeight: 600,
                                color: ar > 0 ? '#D97706' : 'var(--text-faint)',
                              }}
                            >
                              {ar}
                            </span>
                          ...
                          <td>
                            <span
                              style={{
                                color: avgDelayH > 0 ? '#DC2626' : '#059669',
                                fontWeight: 600,
                              }}
                            >
                              {avgDelayH > 0
                                ? `+${fmtBizHours(avgDelayH)}`
                                : '—'}
                            </span>
                          </td>
                          <td>
                  ...
                  <div
                    ...
                    onClick={() =>
                      setDrill({
                        ...
                        rows: filtered.filter(
                          (e) =>
                            (e.record as any).projectName === p.name &&
                            e.sla.status === 'delayed'
                        ),
                      })
                    }
                ...
                {byResponsible.length === 0 ? (
                  ...
                  byResponsible.map((r) => (
                    <div
                      ...
                      onClick={() =>
                        setDrill({
                          title: `SLA Breach — ${r.name}`,
                          rows: filtered.filter(
                            (e) =>
                              e.responsible === r.name &&
                              e.sla.status === 'delayed'
                          ),
                        })
                      }
        ...
        <div>
          ...
          >
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {PROCUREMENT_STAGES.filter(
                (s) => s.number > 0 && s.number <= 22
              ).map((s) => {
                const atStage = filtered.filter(
                  (e) => e.record.currentStage === s.number
                );
                const delCnt = atStage.filter(
                  (e) => e.sla.status === 'delayed'
                ).length;
                const riskCnt = atStage.filter(
                  (e) => e.sla.status === 'at_risk'
                ).length;
                ...
                return (
                  <div
                    ...
                    onClick={() =>
                      delCnt > 0 &&
                      setDrill({
                        title: `Delayed — ${s.name}`,
                        rows: atStage.filter((e) => e.sla.status === 'delayed'),
                      })
                    }
                    ...
                    style={{
                      ...
                      background: bg,
                      color: textC,
                      cursor: delCnt > 0 ? 'pointer' : 'default',
                      minWidth: 78,
                      textAlign: 'center',
                    }}
                  >
        ...
        <div>
          <SectionTitle>
            Management View — SLA Breach Detail (Top 20)
          </SectionTitle>
          <div
            style={{
              background: 'var(--card)',
                      ...
                      return (
                        <tr
                          ...
                          onMouseEnter={(e) => {
                            (
                              e.currentTarget as HTMLTableRowElement
                            ).style.background = 'rgba(15,123,69,0.05)';
                          }}
                          onMouseLeave={(e) => {
                            (
                              e.currentTarget as HTMLTableRowElement
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            {[
              ...
              {
                label: 'Indent Lifecycle',
                href: '/indent-lifecycle',
                icon: TrendingUp,
              },
              {
              ...
              <a
                ...
                style={{
                  ...
                  padding: '9px 16px',
                  borderRadius: 10,
                  border: `1px solid ${href === '/pending-delays' ? 'var(--primary)' : 'var(--border)'}`,
                  background: 'var(--card)',
                  textDecoration: 'none',
                  fontSize: 13,
                  fontWeight: 600,
