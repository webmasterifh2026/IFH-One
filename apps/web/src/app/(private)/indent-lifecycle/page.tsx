...
function computeRecordSLA(r: ReportRecord): SLAResult {
  ...
  const assignedAt = new Date(
    new Date(r.createdAt).getTime() + bizOffset * 3600000
  );
...
function mockFMS(r: ReportRecord, idx: number) {
  ...
  const planDate = new Date(
    new Date(r.createdAt).getTime() + ((i % 7) + 3) * 86400000
  );
  ...
  return {
    ...
    filledBy:
      r.requestedBy?.fullName || RESPONSIBLES[(i + 2) % RESPONSIBLES.length],
    planned: planDate,
    actual: actualDate,
    delayDays,
    pendingDays,
    ...
    requiredDate: rec.requiredDate
      ? new Date(rec.requiredDate)
      : new Date(new Date(r.createdAt).getTime() + (30 + (i % 30)) * 86400000),
    ...
    warrantyGuarantee:
      rec.warrantyGuarantee ||
      (i % 4 === 0 ? '12 Months' : i % 4 === 1 ? '18 Months' : '—'),
    ga: rec.ga || (i % 5 === 0 ? 'Required' : '—'),
    // PO/GRN numbers: use vendorName as indicator or stage-based placeholder
    poNumber:
      r.currentStage >= 6 && rec.vendorName
        ? `${rec.vendorName} / PO-${r.currentStage >= 8 ? 'Approved' : 'Pending'}`
        : r.currentStage >= 6
          ? `PO-2026-${String(i + 1).padStart(4, '0')}`
          : '—',
    grnNumber:
      r.currentStage >= 11 ? `GRN-2026-${String(i + 1).padStart(4, '0')}` : '—',
    indentRemarks: `Standard procurement for ${rec.projectName || 'project site'}`,
    itemType:
      rec.itemType ||
      ['Mechanical', 'Electrical', 'Instrumentation', 'Civil', 'Consumable'][
        i % 5
      ],
  };
}
      ...
      >
        ...
        <div style={{ padding: '16px 20px', flex: 1 }}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            >
              <span>
                Stage {record.currentStage} of 23 —{' '}
                {
                  PROCUREMENT_STAGES.find(
                    (s) => s.number === record.currentStage
                  )?.name
                }
              </span>
          ...
          {[
            ...
            [
              ...
              fms.delayDays === 0
                ? 'On Time'
                : `${fms.delayDays > 0 ? '+' : ''}${fms.delayDays} days`,
            ],
          ...
          >
            {[
              ...
              },
              {
                ...
                href:
                  fms.grnNumber !== '—'
                    ? `/procurement/${record.id}?tab=grn`
                    : undefined,
              },
              {
                label: 'View Attachments',
                href: `/procurement/${record.id}?tab=attachments`,
              },
            ].map((a) => (
              <a
                key={a.label}
                ...
                style={{
                  ...
                  gap: 5,
                  background: 'var(--surface2)',
                  cursor: a.href && a.href !== '#' ? 'pointer' : 'not-allowed',
...
export default function IndentLifecyclePage() {
  ...
  const allOnPageSelected =
    paginated.length > 0 &&
    paginated.every(({ record }) => selectedIds.has(record.id));
  ...
  function exportCSV() {
    const rows = filtered.map(({ record: r, fms }) =>
      [
        ...
        fms.delayDays === 0
          ? 'On Time'
          : `${fms.delayDays > 0 ? '+' : ''}${fms.delayDays}d`,
        fms.poNumber,
        fms.grnNumber,
      ].join(',')
    );
    const header =
      'Indent No,Itemwise Indent No,Filled By,Doer,Responsible Person,Project ID,Project Name,SKU Code,Item Description,Qty,UOM,Tech Spec,Approved Makes,Status,Current Stage,Planned Date,Actual Date,Delay,PO Number,GRN Number';
    const blob = new Blob([[header, ...rows].join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'indent-lifecycle-fms.csv';
    a.click();
    ...
    <div style={{ minHeight: '100vh', background: 'var(--surface2)' }}>
      ...
      >
        ...
        <div ref={dropRef} style={{ position: 'relative' }}>
          <button
            style={btnStyle(stepFilter !== null)}
            onClick={() => setStepDropOpen((v) => !v)}
          >
            <Filter style={{ width: 13, height: 13 }} />
            Filter By Steps{' '}
            {stepFilter !== null &&
              `(${STEP_FILTERS.find((s) => s.stage === stepFilter)?.label})`}
            ...
            >
              <button
                ...
                style={{
                  ...
                  borderRadius: 6,
                  fontSize: 12,
                  fontWeight: stepFilter === null ? 700 : 400,
                  color:
                    stepFilter === null
                      ? 'var(--primary)'
                      : 'var(--text-primary)',
              ...
              {STEP_FILTERS.map((s) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 6,
                    fontSize: 12,
                    fontWeight: stepFilter === s.stage ? 700 : 400,
                    color:
                      stepFilter === s.stage
                        ? 'var(--primary)'
                        : 'var(--text-primary)',
                    background:
                      stepFilter === s.stage ? 'var(--primary-light)' : 'none',
                    border: 'none',
                    cursor: 'pointer',
                  }}
                >
        ...
        {selCount > 0 && (
          <>
            ...
            >
              <Edit2 style={{ width: 13, height: 13 }} /> Edit Selected (
              {selCount})
            </button>
            {selWithPO > 0 && (
              <button
                ...
                onClick={() => setAmendPoOpen(true)}
              >
                <FileText style={{ width: 13, height: 13 }} /> Amend PO Number (
                {selWithPO})
              </button>
            )}
          </>
          ...
          <tbody>
            {paginated.length === 0 ? (
              ...
              paginated.map(({ record: r, fms }, ri) => {
                ...
                const stageName =
                  PROCUREMENT_STAGES.find((s) => s.number === r.currentStage)
                ...
                return (
                  <tr
                    ...
                    onMouseEnter={(e) => {
                      ...
                      ).style.background = 'rgba(15,123,69,0.06)';
                    }}
                    onMouseLeave={(e) => {
                      (
                        e.currentTarget as HTMLTableRowElement
                  ...
                  >
                    ...
                    <Td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button
                          onClick={() => setDetailRecord({ record: r, fms })}
                        ...
                        style={{
                          color:
                            fms.pendingDays > 14
                              ? '#DC2626'
                              : 'var(--text-primary)',
                          fontWeight: fms.pendingDays > 14 ? 700 : 400,
                        }}
                      >
                        {fms.pendingDays}d
                      </span>
                        ...
                        style={{
                          color:
                            fms.poNumber !== '—'
                              ? 'var(--primary)'
                              : 'var(--text-faint)',
                          fontWeight: fms.poNumber !== '—' ? 700 : 400,
                        }}
                      >
                        {fms.poNumber}
                      </span>
                        ...
                        style={{
                          color:
                            fms.grnNumber !== '—'
                              ? '#059669'
                              : 'var(--text-faint)',
                          fontWeight: fms.grnNumber !== '—' ? 700 : 400,
                        }}
                      >
                        {fms.grnNumber}
                      </span>
      ...
      >
        <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
          {selCount > 0 && (
            <strong style={{ color: 'var(--primary)', marginRight: 12 }}>
              {selCount} selected
            </strong>
          )}
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[
            {
              icon: <ChevronsLeft style={{ width: 13, height: 13 }} />,
              action: () => setPage(1),
              disabled: page === 1,
              title: 'First',
            },
          ...
          {[
            {
              icon: <ChevronRight style={{ width: 13, height: 13 }} />,
              action: () => setPage((p) => Math.min(totalPages, p + 1)),
              disabled: page === totalPages,
              title: 'Next',
            },
            {
              icon: <ChevronsRight style={{ width: 13, height: 13 }} />,
