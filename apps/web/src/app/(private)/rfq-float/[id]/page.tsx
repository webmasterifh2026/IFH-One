...
function OverviewTab({ rfqFloat }: { rfqFloat: any }) {
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-700 mt-1">
              {new Date(rfqFloat.rfqDate).toLocaleDateString('en-IN')}
            </p>
          </div>
          <div>
            <p className="text-xs font-semibold text-gray-500 uppercase">
            ...
            <p className="text-sm text-gray-700 mt-1">
              {rfqFloat.submissionDeadline
                ? new Date(rfqFloat.submissionDeadline).toLocaleDateString(
                    'en-IN'
                  )
                : '—'}
            </p>
          ...
          <div>
            ...
            <p className="text-sm text-gray-700 mt-1">
              {rfqFloat.expectedDeliveryDate
                ? new Date(rfqFloat.expectedDeliveryDate).toLocaleDateString(
                    'en-IN'
                  )
                : '—'}
            </p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">
          Products ({rfqFloat.items?.length || 0})
        </h3>
              ...
              {rfqFloat.items?.map((item: any) => (
                <tr key={item.id} className="border-t border-gray-100">
                  ...
                  <td className="px-3 py-2 text-right">
                    {item.unitWeight
                      ? Number(item.unitWeight).toLocaleString()
                      : '—'}
                  </td>
                  <td className="px-3 py-2 text-right">
                    {item.totalWeight
                      ? Number(item.totalWeight).toLocaleString()
                      : '—'}
                  </td>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">
          Vendors ({rfqFloat.vendors?.length || 0})
        </h3>
...
function TCETab({ rfqFloatId }: { rfqFloatId: string }) {
  ...
  if (loading)
    return (
      <div className="text-center py-8 text-gray-500">Loading TCE data...</div>
    );
          ...
          {comparison.highlights?.lowestGrandTotal && (
            <div className="bg-emerald-50 rounded-lg p-3">
              ...
              <p className="text-lg font-bold text-emerald-700">
                {comparison.highlights.lowestGrandTotal.vendor}
              </p>
              <p className="text-sm text-emerald-600">
                ₹
                {Number(
                  comparison.highlights.lowestGrandTotal.total
          ...
          <thead>
            <tr className="bg-gray-50">
              ...
              {comparison.vendors?.map((v: any) => (
                <th
                  key={v.tceId}
                  className="px-3 py-2 text-center text-xs font-semibold text-gray-500 uppercase border-l"
                >
                  {v.vendorName}
                  <span className="block text-[10px] font-normal text-gray-400">
                    ₹
                    {v.grandTotal
                      ? Number(v.grandTotal).toLocaleString('en-IN')
                      : '—'}
                  </span>
                </th>
              ))}
              ...
              <tr key={product.productId} className="border-t border-gray-100">
                <td className="px-3 py-3 font-medium text-gray-700">
                  ...
                  <span className="block text-xs text-gray-500">
                    {product.itemCode} · Qty:{' '}
                    {Number(product.quantity).toLocaleString()} {product.uom}
                  </span>
                  {product.totalWeight && (
                    <span className="block text-xs text-gray-500 mt-1">
                      Wt: {product.totalWeight} ({product.unitWeight}/unit)
                    </span>
                  )}
                  {product.itemRemarks && (
                    <span className="block text-xs text-amber-600 mt-1">
                      Note: {product.itemRemarks}
                    </span>
                  )}
                </td>
                {comparison.vendors?.map((v: any) => {
                  ...
                  return (
                    <td
                      key={v.tceId}
                      className={`px-3 py-3 text-center border-l ${isLowest ? 'bg-emerald-50' : ''}`}
                    >
                      {vData ? (
                        <>
                          <p
                            className={`font-semibold ${isLowest ? 'text-emerald-600' : 'text-gray-900'}`}
                          >
                            ₹{Number(vData.rate || 0).toLocaleString('en-IN')}
                          </p>
                          <p className="text-xs text-gray-500">
                            Disc: {vData.discount || 0}%
                          </p>
                          <p className="text-xs text-gray-500">
                            GST: {vData.gst || 0}%
                          </p>
  ...
  if (negotiations.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
        ...
        <p className="text-sm text-gray-500">
          Start negotiation after receiving vendor quotations.
        </p>
      </div>
    );
  }
                ...
                <tbody>
                  {neg.items.map((item: any) => (
                    <tr key={item.id} className="border-t border-gray-100">
                      ...
                      <td className="px-3 py-2 text-right">
                        {item.originalRate
                          ? `₹${Number(item.originalRate).toLocaleString('en-IN')}`
                          : '—'}
                      </td>
                      <td className="px-3 py-2 text-right font-medium text-blue-600">
                        {item.negotiatedRate
                          ? `₹${Number(item.negotiatedRate).toLocaleString('en-IN')}`
                          : '—'}
                      </td>
                      <td className="px-3 py-2 text-right font-bold text-emerald-600">
                        {item.finalRate
                          ? `₹${Number(item.finalRate).toLocaleString('en-IN')}`
                          : '—'}
                      </td>
                      <td className="px-3 py-2 text-right">
