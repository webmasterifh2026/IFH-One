...
export default function ControlTowerPage() {
  ...
  const pending = records.filter((r) =>
    ['DRAFT', 'IN_PROGRESS', 'SUBMITTED'].includes(r.status)
  ).length;
  ...
  const fulfilled = records.filter(
    (r) => r.status === 'COMPLETED' && r.currentStage === 2
  ).length;
  const paid = records.filter(
    (r) => r.status === 'COMPLETED' && r.currentStage === 23
  ).length;
  ...
  const slaDelayedToday = slaDelayed.filter(
    (d) =>
      d.sla.deadline.getTime() > now - dayMs && d.sla.deadline.getTime() <= now
  );
  const slaDelayedWeek = slaDelayed.filter(
    (d) =>
      d.sla.deadline.getTime() > now - 7 * dayMs &&
      d.sla.deadline.getTime() <= now
  );
  const slaOnTime = slaData.filter((d) => d.sla.status === 'on_time');
  const avgCompliance =
    slaData.length > 0
      ? Math.round((slaOnTime.length / slaData.length) * 100)
      : 100;
  const avgBizHrs =
    slaData.length > 0
    ...
    <div className="page-content">
      ...
      >
        ...
        {[
          {
            label: 'Total Delayed',
            value: slaDelayed.length,
            color: '#DC2626',
            href: '/pending-delays',
          },
          {
            label: 'Delayed Today',
            value: slaDelayedToday.length,
            color: '#EA580C',
            href: '/pending-delays',
          },
          {
            label: 'Delayed This Week',
            value: slaDelayedWeek.length,
            color: '#B45309',
            href: '/pending-delays',
          },
          {
        ...
        >
          Mon–Fri + 2nd/4th/5th Sat · 09:00–18:00
        </span>
      </div>
      ...
      >
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        fontWeight: 700,
                        color:
                          count > 0 ? 'var(--primary)' : 'var(--text-muted)',
                      }}
                    >
                      {count}
                    </span>
        ...
        <div className="ifh-card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          >
            ...
            >
              Current stage · responsible person · planned vs actual · delay
            </p>
          </div>
            ...
            <table className="ifh-table">
                ...
                {recent.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td>
                        {sla
                          ? sla.assignedAt.toLocaleDateString('en-IN', {
                              day: 'numeric',
                              month: 'short',
                            })
                          : '—'}
                      ...
                      <td>
                        {sla
                          ? sla.deadline.toLocaleDateString('en-IN', {
                              day: 'numeric',
                              month: 'short',
                            })
                          : '—'}
              ...
              <tfoot>
                <tr>
                  ...
                  >
                    Showing 10 most recent · business TAT engine: Mon–Fri +
                    2nd/4th/5th Sat · 09:00–18:00
                  </td>
                </tr>
              </tfoot>
            </table>
