...
export default function PermissionsPage() {
      ...
      await Promise.all(
        roleList.map(async (r) => {
          try {
            ...
            rp[r.id] = new Set(
              (Array.isArray(rPerms) ? rPerms : []).map((p: Permission) => p.id)
            );
          } catch {
            rp[r.id] = new Set();
          }
  ...
  return (
    <div className="page-content">
                ...
                {filteredPerms.length === 0 ? (
                  ...
                  filteredPerms.map((p, idx) => (
                    <tr
                      ...
                      style={{
                        background:
                          idx % 2 === 0 ? 'var(--surface1)' : 'var(--surface2)',
      ...
      >
        <span>
          {filteredPerms.length} of {permissions.length} permissions shown
        </span>
