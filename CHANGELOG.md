...
### Added
- Complete automated single-source-of-truth versioning release pipeline for v3.0.3.
- Automated GitHub release notes and annotated git tag generation.

### Security & Cleanup
- Purged all hardcoded version environment variables (`APP_VERSION`, `RELEASE_VERSION`, `BUILD_VERSION`).
- Sanitized committed Neon PostgreSQL connection strings and secrets across all `.env.example` templates.

## [3.0.2] - 2026-07-23

### Added
- Centralized application version management with root `package.json` as single source of truth.
- Automated version synchronization script (`npm run version:sync`).
- Dedicated Backend Version API endpoints (`GET /api/version` and `GET /version`).
- Swagger/OpenAPI documentation integration with `v3.0.2` schema.
- System Information card on Profile/Settings page displaying Version, Build Date, Git Commit, and Environment.

### Changed
- Updated Login Page, Home Page, and Dashboard Footer to read dynamic version `v3.0.2`.
- Upgraded GitHub Actions CI/CD workflows to Node.js 22.x LTS.
- Hardened workflow permissions (`contents: read`) and added concurrency cancellation control.
